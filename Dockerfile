...
RUN gosu node mkdir -p /tmp/ghost-storage-adapter-s3 && \
    ...
    gosu node cp -r /tmp/ghost-storage-adapter-s3/node_modules/ghost-storage-adapter-s3/src $GHOST_INSTALL/current/core/server/adapters/storage/s3 && \
    gosu node rm -r /tmp/ghost-storage-adapter-s3 && \
    gosu node yarn add aws-sdk